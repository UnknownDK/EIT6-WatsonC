...
CSL_Status reader_init(circular_dma_reader_handle * handle, circular_dma_reader_config * config) {
    ...
    handle->dmaConfig.dataLen = 4000;//config->buffer_len * 4; // DMA works in "byte-addressing space" and does not use CPU 16-bit addressing intervals. A 32 bit array element is therefore counted as 4 * 8 bits.
